...
import BusManageBookingPage from "./bus-manage-booking-page"
import HireManageBookingPage from "./hire-manage-booking-page"
import {
  ChevronRight,
  SearchX,
  // Bus,
...
const ManageBookingPage = () => {
  ...
  const busBooking = getBookingByRef(ref)
  // Check if it's a hire booking
  const hireBooking = getHireBookingByRef(ref)

  if (busBooking) {
    return <BusManageBookingPage />
  } else if (hireBooking) {
    return <HireManageBookingPage />
  } else {
    // Neither found
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-lg flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mb-10 text-base leading-relaxed text-on-surface-variant">
          ...
          Please double-check the reference number or book a new trip/charter.
        </p>
        <button
          onClick={() => navigate({ to: "/" })}
          className="group flex w-full max-w-70 items-center justify-center gap-3 rounded-full bg-primary px-8 py-4 font-bold tracking-widest text-on-primary uppercase shadow-lg shadow-primary/20 transition-all hover:scale-[1.02] active:scale-[0.98]"
        >
          Book Now
          <ChevronRight
            size={18}
